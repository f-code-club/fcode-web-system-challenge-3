      ...
      <section className="col-span-1 lg:col-span-8" id="members">
        <div className="overflow-hidden rounded-lg border border-gray-200/70 bg-white shadow-xs transition-all">
          <div className="border-b border-gray-200/70 bg-linear-to-r from-gray-50/80 to-white px-5 py-4 sm:px-6 sm:py-5">
            <h2 className="text-base font-semibold tracking-tight text-gray-900 sm:text-lg">DANH SÁCH CHẤM</h2>
              ...
              <tbody className="divide-y divide-gray-200/60 bg-white">
                {rooms && rooms.length > 0 ? (
                  rooms.map((room, index) => {
                    const { color, status, text } = Helper.formatTimeRange(room.startTime, room.endTime);
                    return (
                      <tr
                        key={room.id}
                      ...
                      >
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex-1">
                              <h2 className="text-base font-medium tracking-tight text-gray-900">
                                [NHÓM <span className="text-primary font-medium">{room.team?.group}</span>] -{' '}
                                {room.team?.name ? (
                              ...
                              <span className="text-xs italic">
                                Đề tài:{' '}
                                <span className="text-primary">{room.team?.topic?.title || 'Chưa có đề tài'}</span>
                              </span>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3.5 text-sm whitespace-nowrap text-gray-600 sm:px-6 sm:py-4">
                          ...
                          <p className="mt-0.5 text-xs text-gray-500">
                            Thuyết trình {room.presentPhase === 'OFFICIAL' ? 'chính thức' : 'thử'}
                          </p>
                        </td>
                        ...
                        <td
                          ...
                          style={{ color }}
                        >
                          <span className="text-sm font-bold whitespace-nowrap md:table-cell" style={{ color }}>
                            {' '}
                            {text}
                          </span>
                        ...
                        <td className="px-4 py-3.5 text-sm text-gray-600 sm:px-6 sm:py-4">
                          {room.presentPhase == 'OFFICIAL' ? (
                            <Button variant="outline" size="sm" className="w-fit text-xs" asChild>
                              <Link to={`/judge/room/${room.id}/judge/${user.id}`} className="flex items-center gap-1">
                                <span>Chi tiết</span>
                              </Link>
                            </Button>
                          ) : (
