      ...
      <div className="overflow-hidden rounded-lg border border-gray-200/70 bg-white">
        <div className="border-b border-gray-200/70 bg-linear-to-r from-gray-50/80 to-white px-5 py-4 sm:px-8 sm:py-5">
          ...
          <p className="mt-1.5 text-xs leading-relaxed text-gray-500 sm:text-sm">
            Lịch trình thực hiện Challenge Vòng 3. Timeline cũng có thể thay đổi, chúng tôi sẽ thông báo trên Discord.
          </p>
        </div>
          ...
          <div className="relative mt-2 space-y-8">
            ...
            {timelineData.map((item, index) => (
              <div key={index} className="relative flex gap-5">
                <Dot number={index + 1} active={Helper.isInRangeDate(new Date(), item.startDate, item.endDate)} />

                {/* Content */}
                <Content {...item} active={Helper.isInRangeDate(new Date(), item.startDate, item.endDate)} />
              </div>
            ))}
          </div>
        </div>
