          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                {data &&
                  !data.name &&
                  user.id === data.candidates.find((candidate) => candidate.id === data.leaderId)?.user.id && (
                    <ChangeNameTeam team={data} />
                  )}
              </div>
              {!data?.name && (
                <p className="mt-1 text-xs text-red-500 sm:text-sm">Nhóm trưởng mới được quyền đặt tên nhóm</p>
              )}
              <p className="mt-1.5 text-xs leading-relaxed text-gray-500 sm:text-sm">
                Danh sách thành viên trong nhóm, vui lòng chủ động liên hệ mentor và các thành viên trong nhóm thông qua
                kênh <span className="font-bold">Discord</span>.
              </p>
            </div>
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {!user.isConfirm && (
                        <p className="mt-0.5 text-xs font-semibold text-red-600">Chưa đăng nhập lần đầu!</p>
                      )}
                    ...
                    <td className="px-4 py-3.5 text-sm whitespace-nowrap text-gray-600 sm:table-cell sm:px-6 sm:py-4">
                      <p className="text-blue-gray-900 text-sm font-semibold">{member.studentCode}</p>
