      ...
      <form onSubmit={handleSubmit} className="space-y-6 p-5 sm:p-6">
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-600">
            Chọn <span className="font-semibold">MỘT</span> khung giờ để thuyết trình thử nghiệm và nhận phản hồi từ Ban
            Giám Khảo.
          </p>
          {isTrialFullyBooked ? (
            <div className="rounded-md border-l-4 border-red-400 bg-red-50 p-3">
              ...
              <p className="mt-1 text-xs text-red-700">
                Tất cả 10 slot thuyết trình thử đã được đăng ký đầy. Bạn vẫn có thể đăng ký thuyết trình chính thức bên
                dưới.
              </p>
            </div>
          ) : (
            <div className="rounded-md border-l-4 border-blue-400 bg-blue-50 p-3">
              <p className="text-xs text-blue-800">
                ...
                <span className="font-bold">{10 - disabledTrialSlotsCount} slot</span> thuyết trình thử. Hãy chọn một
                khung giờ phù hợp có đủ tất cả các thành viên trong nhóm để tham gia.
              </p>
            </div>
          )}
                ...
                <div key={`trial-${schedule.date}`} className="mb-5 space-y-3">
                  ...
                  <div className="grid gap-2 pl-6 sm:grid-cols-2 lg:grid-cols-4">
                    {schedule.slots.map((slot) => (
                      <div key={`trial-${schedule.date}-${slot.time}`} className="flex items-center space-x-2">
                        <RadioGroupItem
                        ...
                        >
                          {slot.time}
                          {slot.disabled && <span className="ml-1 text-xs text-red-500">(Hết)</span>}
                        </Label>
                      </div>
                    ))}
                  </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-600">
            Chọn <span className="font-semibold text-green-700">NHIỀU</span> khung giờ bạn có thể tham gia thuyết trình
            chính thức. BTC sẽ sắp xếp và thông báo lịch cụ thể sau.
          </p>
          <div className="rounded-md border-l-4 border-green-400 bg-green-50 p-3">
            <p className="text-xs text-green-800">
              <span className="font-semibold">Lưu ý:</span> Tất cả các nhóm đều được quyền tham gia thuyết trình chính
              thức. Vui lòng chọn <span className="font-bold">TẤT CẢ</span> các khung giờ mà{' '}
            ...
            {officialSchedules.map((schedule) => (
              <div key={`official-${schedule.date}`} className="space-y-3">
                ...
                <div className="grid gap-3 pl-6 sm:grid-cols-2 lg:grid-cols-4">
                  {schedule.slots.map((slot) => (
                    <div key={`official-${schedule.date}-${slot.time}`} className="flex items-center space-x-2">
                      <Checkbox
                        id={`official-${schedule.date}-${slot.time}`}
                        checked={officialSlots.includes(`${schedule.date}|${slot.time}`)}
                        onCheckedChange={() => handleOfficialSlotChange(`${schedule.date}|${slot.time}`)}
                      />
                      <Label
        ...
        <div className="flex items-center gap-3 border-t border-gray-200/70 pt-5">
          ...
          <p className="text-xs text-gray-500">
            <span className="font-semibold text-red-600">Lưu ý:</span> Bạn phải chọn ít nhất 1 khung giờ cho thuyết
            trình chính thức
          </p>
        </div>
      </form>
