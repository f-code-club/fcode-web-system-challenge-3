...
import { CheckCircle2, ChevronDown, ChevronUp, Clock, Trash2, XCircle } from 'lucide-react';
                  ...
                  <div className="flex-1">
                    <div className="mb-2 flex flex-col gap-2 text-sm font-medium">
                      ...
                      <span className="text-xs italic">
                        Đề tài: <span className="text-primary">{room.team?.topic?.title || 'Chưa có đề tài'}</span>
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-1 text-xs" style={{ color }}>
                        ...
                        <span className="text-gray-500">- {Helper.formatDateTime(room.startTime)}</span>
                      </div>
                      {/* {room.teamScore !== null && (
                ...
                {isExpanded && (
                  <div className="mt-4 rounded-lg border border-gray-200/70 bg-gray-50/30">
                    {isLoadingDetail ? (
                      ...
                      <div className="px-4 py-8 text-center text-sm text-gray-500">Chưa có giám khảo nào</div>
                    ) : (
                                  ...
                                  <td className="px-4 py-3.5 sm:px-6 sm:py-4">
                                    <ul className="list-disc">
                                      {jr.candidateScores.map((cs) => (
                                        <li key={cs.candidateId} className="text-sm text-gray-700">
                                          {cs.candidateName}:{' '}
                                          <span
                                            className={`font-semibold ${
                                              Helper.belowAverage(cs.score) ? 'text-red-500' : 'text-green-500'
                                            }`}
                                          >
                                            {cs.score}
                                          </span>
